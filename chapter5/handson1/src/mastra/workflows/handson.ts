...
export const handsonWorkflow = createWorkflow({
    id: "handsonWorkflow",
    description: "自然言語の質問からConfluenceの要件書を検索し、内容を要約して取得します。",
    inputSchema: z.object({
        query: z
            .string()
            .describe(
                "検索したい内容を自然言語で入力してください（例：「AIに付いての情報」「最新のプロジェクト情報」）",
            ),
        }),
    outputSchema: z.object({
        text: z.string().describe("要約された内容"),
    }),
})
.then(
    createStep({
...
.then(
    createStep({
        ...
        execute: async ({ inputData, getInitData }) => {
            // 一つ前のステップのoutputSchemaから渡されたデータ
